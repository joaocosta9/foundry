...
impl ScriptArgs {
    pub async fn run_script(self) -> eyre::Result<()> {
        ...
        let script_config =
            ScriptConfig { config: Config::from_provider(figment).sanitized(), evm_opts };
        ...
        let nonce = if let (Some(deployer), Some(fork_url)) =
            (self.deployer, &script_config.evm_opts.fork_url)
        {
        ...
        let BuildOutput {
            ...
            sources,
        } = self.build(&script_config, self.deployer, nonce)?;
        ...
        if self.force_resume || self.resume {
            ...
            let mut result =
                self.execute(&script_config, contract, self.deployer, &predeploy_libraries).await?;
            ...
            if self.debug {
                ...
                if let Some(new_sender) = self.maybe_new_sender(
                    ...
                    known_contracts = self
                        .rerun_with_new_deployer(
                            project,
                            &script_config,
                            script_config.evm_opts.fork_url.as_ref().expect(
                                "You must provide an RPC URL (see --fork-url) when broadcasting.",
                            ),
                            new_sender,
                            &mut result,
                ...
                self.show_traces(&decoder, script_config.evm_opts.verbosity, &mut result)?;
